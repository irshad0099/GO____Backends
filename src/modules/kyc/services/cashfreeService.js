...
/**
 * Fetch a specific document from DigiLocker after user has granted consent
 * document_type: 'AADHAAR' | 'PAN' | 'DRIVING_LICENSE'
 */
export const getDigilockerDocument = async (documentType, verificationId, referenceId) => {
    try {
        const params = {};
        ...
        const res = await axios.get(
            `${CF_BASE}/digilocker/document/${documentType}`,
            { headers: cfHeaders(), params }
        );
        return res.data;
    } catch (err) {
        logCfError('getDigilockerDocument', err);
        throw err;
    }
};

/**
 * Get the current status of a DigiLocker verification request
 * Status: PENDING | AUTHENTICATED | EXPIRED | CONSENT_DENIED
 */
export const getDigilockerStatus = async (verificationId, referenceId) => {
    try {
        const params = {};
        if (verificationId) params.verification_id = verificationId;
        if (referenceId)    params.reference_id    = referenceId;
        ...
        logCfError('getDigilockerStatus', err);
        throw err;
    }
};

/**
 * Create a DigiLocker URL to retrieve and verify Aadhaar, PAN, DL
 */
export const createDigilockerUrl = async (verificationId, documentRequested, redirectUrl, userFlow) => {
    try {
        const res = await axios.post(
            `${CF_BASE}/digilocker`,
            {
        ...
        logCfError('createDigilockerUrl', err);
        throw err;
    }
};

/**
 * Check if a mobile number or Aadhaar is registered with DigiLocker
 */
export const verifyDigilockerAccount = async (mobileNumber, aadhaarNumber) => {
    try {
        const body = { verification_id: `digi_${Date.now()}` };
        if (mobileNumber)  body.mobile_number  = mobileNumber;
        if (aadhaarNumber) body.aadhaar_number = aadhaarNumber;

        const res = await axios.post(
            `${CF_BASE}/digilocker/verify-account`,
            body,
            { headers: cfHeaders() }
        );
        return res.data;
    } catch (err) {
        logCfError('verifyDigilockerAccount', err);
        throw err;
    }
};
